...
class ControlBD():
    def __init__(self):
        ...
        self.connection = sqlite3.connect('BD/people.BD')
        ...
        self.cursor.execute('''
            CREATE TABLE IF NOT EXISTS Emails (
                id_people INTEGER PRIMARY KEY, -- id
                id_ INTEGER NOT NULL CHECK, -- id пользователя
    ...
    def update_BD(self, BD, id, column_name, new_value):
        ...
        if column_name is not None:
            ...
            self.cursor.execute(f"UPDATE ? SET ? = ? WHERE id = ?",(BD,column_name, new_value, id,))
            self.connection.commit()

        return {'success':True, 'massege': 'Изменения успешно внесяны'}


    # Функция контроля друзей
    def control_friendsBD(self, friend_1, friend_2, start_end_being_friends):
        ...
        self.update_BD(friend_1, 'friends', json.dumps(list_friend_1))
        self.update_BD(friend_2, 'friends', json.dumps(list_friend_2))

        return {'success':True, 'massege': massege}
    
    
    # Функция вывода списка друзей пользователя
    def list_friendsBD(self, id):
